# Kinder Signs Risk Facts

**Frozen baseline:** 8eb0742
**Use:** Canonical evidence and wording for risk, pilot, and presentation documents

This file does not certify product safety, legal compliance, sign correctness, or pilot readiness.

## Evidence rules

- A technical extraction pass is not sign approval.
- Detection coverage is not sign accuracy.
- A registry hash is not proof of ownership, consent, or complete security.
- An importable n8n export is not a live workflow run.
- A LangSmith dry-run is not a live external trace.
- A separately prepared Gemini video is not current landmark-generated output.
- A screen prototype is not production delivery.
- A family preview is not a personalised assignment-driven family library.
- A successful schema or hash check is not publication approval.

## Technical evidence

The committed Round 1 WATER run records:

- 332 processed frames;
- 100.00 percent pose coverage;
- 93.98 percent dominant right-hand coverage;
- 20 missing hand frames;
- extraction pass; and
- motion representation partial.

The 93.98 percent value is hand-detection coverage for WATER. It is not sign correctness.

The registered MORE input is present and hash-mapped. The canonical registry does not assign the WATER diagnostics to MORE. A current MORE run with 285 frames, 100 percent pose coverage, and 91.93 percent dominant-hand coverage exists only as ignored local-run evidence and must be labelled local-only if cited.

## Current asset facts

- The canonical registry covers MORE, HELP, EAT, SLEEP, MILK, and WATER.
- Each sign has three deterministic Open Peeps-derived SVG candidates.
- All 18 visual candidates need human sign and visual review.
- No current sign is registered as printable, school-available, or published.
- Open Peeps by Pablo Stanley supplies the CC0-recorded base character and line grammar only.
- Reviewed functional references, sign knowledge, frames, and movement evidence determine mechanics.
- Founder-provided references, the current context image, and Gemini files retain purpose-specific rights questions.

The character defines the look. The reviewed reference defines the sign.

## Gemini facts

| Sign | Current demo file | Boundary |
| --- | --- | --- |
| MORE | mas.mp4 | Separate pre-generated demo |
| HELP | ayuda.mp4 | Separate pre-generated demo |
| MILK | leche.mp4 | Separate pre-generated demo |
| EAT | None | Static flow remains available |
| SLEEP | None | Static flow remains available |
| WATER | None | Static flow remains available |

Rights and external-display permission remain open. The files are not linguistically certified. They were not generated from the current MediaPipe run or its landmarks.

The local service checks file identity and mapping. It accepts a null demo-display permission as locally available, so the rights boundary is not fully fail-closed.

## Current and pilot risks

| Risk | Why it matters | Current control | Evidence state | Pilot gate |
| --- | --- | --- | --- | --- |
| Sign mechanics or hand articulation is wrong | Families may repeat incorrect material | Source hierarchy, draft status, human review UX | Operational gap | Qualified sign approval for every pilot asset |
| Detection metric becomes a correctness claim | Technical evidence can mislead reviewers or buyers | Coverage and partial-motion labels | Evidence present | Review every UI, document, and sales statement |
| Source or performer rights are missing | Use or distribution may be unauthorised | Registry classifications and hashes | Partial evidence | Written authority, usage scope, expiry, and withdrawal |
| Gemini video is misrepresented or displayed without permission | Creates rights, trust, and transparency risk | Separate asset class and local disclosure | Material gap | No external display until rights, fidelity, and Article 50 decisions close |
| Six-sign registry and five-sign package disagree | The wrong sign can inherit evidence or publication data | Canonical registry identifies WATER correctly | Present contradiction | Regenerate package with zero cross-sign evidence links |
| Direct URL retrieves unsuitable content | Transport controls do not establish lawful or safe use | Public-target, redirect, MIME, size, time, filename, cleanup, and redaction controls | Partial evidence | Authorised sources, HTTPS or controlled storage, identity, rate, isolation, logs, and deletion |
| Child or family data enters an AI path | Vulnerable-person data could be exposed or reused | Synthetic data and no live personal-data model call | Current evidence present; future risk | Zero personal child or family data sent to LLM or LangSmith |
| Wrong family sees an assignment | Reveals a child relationship and damages trust | No real delivery in current prototype | Cannot be tested yet | Secure identity, nursery separation, relationship checks, and deny-by-default access |
| Unreviewed content reaches a user | Removes the principal safety control | Current registry blocks all signs; review UX exists | Partial evidence | Authenticated publication gate and zero unreviewed delivery |
| Unsupported family wording sounds clinical | May create unsafe expectations | Banned-claim and source-adherence checks | Partial evidence | Live-provider evaluation and human approval |
| Reviewer decisions vary | Quality and throughput may be unstable | Review prompts | Measurement gap | Training, rubric, second review, disagreement and exception measures |
| Correction does not propagate | Harm persists after discovery | Versioned assets only | Operational gap | Complaint, recall, recipient notification, and withdrawal drill |
| Accessibility is insufficient | Families may not understand or use material | Spanish and bilingual formats | Partial evidence | Keyboard, screen-reader, contrast, print, caption, and comprehension testing |
| Energy or carbon benefit is overstated | Creates unsupported sustainability claims | Green AI audit states measurement gap | Evidence present for gap | No benefit claim until functional-unit measurement exists |
| Educators do not repeat assignments | Product may not solve the operational problem | Local assignment prototype | Pilot hypothesis | Measure time, support, repeat use, and abandonment |
| Families do not use or value guidance | Preview does not prove value | Basic Family View | Pilot hypothesis | Measure authorised access, repeat use, clarity, and usefulness |
| Buyers will not pay | A working asset may not support a business | Pricing and ROI scenarios only | Commercial evidence gap | Test decision-maker budget, willingness to pay, and continuation |

## Three priority risks

### 1. Sign fidelity and review feasibility

Client fact: zero production signs are published.

Action: complete rights, sign, visual, and publication review for the pilot set.

Target: three to five signs with complete evidence and no critical content incident.

Owner: Content Operations and a qualified sign reviewer.

Decision rule: stop if reviewers cannot approve the set reliably or at a workable cost.

### 2. Willingness to pay and repeated adoption

Client fact: adjacent family and educator offers are paid, but KinderFlow has zero paying centres and no retention evidence.

Action: test EUR 600, EUR 1,200, and EUR 1,800 with named budget owners; record first use, repeated assignments, support burden, and a price-specific continuation decision.

Target: at least two participating schools identify a budget owner and state credible continuation intent at a tested price, alongside the educator repeat-use target.

Owner: Product Owner, commercial lead, and nursery sponsors.

Decision rule: iterate when users see value but price, packaging, or buyer process remains fixable; stop when no participating school identifies a viable recurring paid path.

### 3. Rights and provenance

Client fact: Open Peeps provenance is recorded, but several reference and Gemini usage rights remain open.

Action: document source authority, display, adaptation, distribution, expiry, and withdrawal for each pilot asset.

Target: 100 percent pilot-sign provenance, including rights and review.

Owner: Product Owner and rights owner.

Decision rule: block every asset with an unknown external-use status.

## Material gate outside the top three: secure family delivery

Client fact: Family View is a local preview, not a personalised account service.

Action: build and test group-first, role-based, nursery-separated family access.

Target: zero cross-family or cross-nursery access and zero personal-data model transfers.

Owner: Engineering, privacy owner, and nursery controller.

Decision rule: stop the pilot on any wrong-recipient access or unauthorised AI transfer.

## Hard pilot boundaries

- child video processed in core scope equals zero;
- child scoring equals zero;
- automated educational decisions equal zero;
- unreviewed content delivered equals zero;
- blocked content delivered equals zero;
- personal child or family data sent to LLM or LangSmith equals zero;
- pilot sign provenance, including rights and review, equals 100 percent; and
- review exceptions with rationale equals 100 percent.

## Evidence

- [WATER POC evidence](../poc/output/)
- [Current MVP reality check](mvp_reality_check.md)
- [Local pipeline](../mvp/pipeline.py)
- [Current tests](../mvp/tests/)
- [Canonical asset registry](../assets/registry/sign_asset_registry.json)
- [Asset inventory](../assets/registry/sign_asset_inventory.md)
- [Open Peeps provenance](../assets/flashcards/open_peeps/provenance.json)
- [Content operations reports](../content_ops/reports/)
- [Responsible AI audit](audits/responsible_ai_audit.md)
- [GDPR record](../compliance/gdpr_documentation.md)
- [Green AI audit](audits/green_ai_audit.md)
